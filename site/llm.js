/**
 * llm.js — a small GPT, written from scratch in plain JavaScript.
 *
 * No dependencies, no framework, no build step, no server, no API key.
 * Everything a language model needs is here: a tokenizer, token and position
 * embeddings, causal multi-head self-attention, a feed-forward block, layer
 * normalisation, residual connections, softmax cross-entropy, hand-derived
 * backpropagation and AdamW — plus sampling with temperature and top-k.
 *
 * It is deliberately small. `quick` is about ten thousand parameters and trains
 * on one CPU core in a few seconds; a frontier model is roughly a hundred
...
export function buildVocab(text, maxVocab = 128) {
  ...
  let chars = [...freq.keys()].sort(
    (a, b) => freq.get(b) - freq.get(a) || (a < b ? -1 : a > b ? 1 : 0)
  );
  let unknown = null;
 ...
 * actually meets — because the browser has to compile the loops before it settles.
 * Measured in Chrome on a desktop CPU, 2026-09-17: 600 `quick` steps in 13.8 s
 * cold and 8 s warm, 250 `standard` steps in 27 s warm, and the biggest model at
 * roughly 0.4 s a step. The page also reports the rate it is really achieving,
 * with an estimate taken from that rate, so a slower machine says so rather than
 * quietly taking longer. Any run can be stopped early and tested as it stands.
 */
...
export function preset(name) {
  return { ...(PRESETS[name] || PRESETS.quick) };
}
...
export class Trainer {
  constructor({ data, vocab, config, seed = 1337 }) {
    ...
    for (const L of this.layers) for (const k in L) this.params.push(L[k]);
  ...
  sample({ prompt = '', length = 240, temperature = 0.8, topK = 0, seed = null } = {}) {
    ...
    const T = this.T;
    const ids = prompt ? Array.from(encode(prompt, this.vocab)) : [lineStartToken(this.data, this.vocab)];
    const out = ids.slice();
  ...
  exportWeights() {
    const params = {};
    for (const p of this.params) params[p.name] = Array.from(p.w, (x) => Number(x.toFixed(6)));
    return {
      name: 'llm-demo',
      architecture:
