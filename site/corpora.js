/**
 * corpora.js — the texts the demo trains on out of the box.
 *
 * The default is deliberately small and highly structured: a list of given
 * names gives a tiny model enough regularity to invent new ones within
 * seconds, which is the honest version of "watch it learn". A larger, messier
 * text needs a bigger model and far more time, and a browser cannot give it
 * that — the page says so rather than pretending otherwise.
 *
 * The dialogue set was written for this demo. The names are a hand-written
 * list of common given names (a list of names is a fact, not a work).
 */
