...
function initAnalytics() {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      track('element_click', {
        element_id: el.id || el.dataset.ga || '',
...
function createLocalTransport() {
  return import('./trainer-host.js').then(
    ({ LlmHost }) => {
      ...
      return {
        send: (message) => host[message.type === 'train' ? 'start' : message.type === 'generate' ? 'generate' : message.type === 'stop' ? 'stop' : message.type === 'export-weights' ? 'exportWeights' : 'exportReport'](message),
        onMessage: (handler) => { currentHandler = handler; },
        onError: () => {},
      };
    }
  );
...
function refreshCorpusStats() {
  ...
  if (state.corpusKey !== 'own' && CORPORA[state.corpusKey]) {
    note.textContent = CORPORA[state.corpusKey].note;
  } else {
    note.textContent = 'Paste anything — a page of prose, a list of product names, a script. Line structure is what a small model learns fastest.';
  }
  refreshModelPreview();
}
...
function onStarted(message) {
  ...
  if (message.truncated) {
    setStatus('That text was longer than the demo will use — the first 200,000 characters are being trained on.', 'warn');
  } else {
    setStatus('Training…');
  }
  track('train_started', { step: 0, parameters: message.parameters });
}

function onProgress(message) {
  state.stepsDone = message.step;
  state.steps = message.steps;
  state.rate = message.stepsPerSecond;
...
function onDone(message, stopped) {
  ...
  for (const id of ['generateBtn', 'downloadWeights', 'downloadReport']) $(id).disabled = false;
...
async function boot() {
  ...
  if (!transport) transport = await createLocalTransport();
  ...
  transport.onMessage((message) => {
    switch (message.type) {
      case 'started': onStarted(message); break;
      case 'progress': onProgress(message); break;
      case 'done': onDone(message, false); break;
      case 'stopped': onDone(message, true); break;
      case 'sample': renderSample(message); break;
      case 'generated': $('genMsg').textContent = `Sampled ${message.count} times in ${message.seconds.toFixed(1)} s.`; break;
      case 'weights': onWeights(message.payload); break;
      case 'report': onReport(message.payload); break;
      case 'error': setStatus(message.message, 'warn'); break;
      default: break;
    }
  });
  ...
  $('downloadWeights').addEventListener('click', () => transport.send({ type: 'export-weights' }));
  $('downloadReport').addEventListener('click', () =>
    transport.send({
      ...
      extra: {
        ...
        preset: state.presetKey,
        samples: [...document.querySelectorAll('#samples .sample')].map((el) => el.textContent),
      },
    })
  );
  ...
  for (const id of ['temperature', 'topK', 'length']) {
    $(id).addEventListener('input', () => {
      $(`${id}Value`).textContent = id === 'temperature' ? Number($(id).value).toFixed(2) : $(id).value;
    });
  }
