...
declare global {
  interface Window {
    ...
    /**
     * Defined by consent.ts, and ONLY once a visitor has allowed analytics. Its
     * absence is the gate: this page has no other way to send an event, so a
     * visitor who has not consented cannot be counted by accident.
     */
    llmTrack?: (name: string, params?: Record<string, unknown>) => void;
  }
}
...
function $<T extends HTMLElement = HTMLElement>(id: string): T {
  ...
  if (!el) throw new Error(`llm-demo: the page has no #${id}`);
...
/* ------------------------------------------------------------------ *
 * Transport — a worker when the browser has module workers, else this thread
 * ------------------------------------------------------------------ */
...
function createWorkerTransport(): Transport | null {
  try {
    ...
    return {
      ...
      onMessage(handler: (message: HostMessage) => void): void {
        worker.onmessage = (event: MessageEvent<HostMessage>) => handler(event.data);
      },
      onError(handler: (message: string) => void): void {
...
async function createLocalTransport(): Promise<Transport> {
...
let currentHandler: ((message: HostMessage) => void) | null = null;
...
const number = (n: number | null | undefined): string =>
  n === null || n === undefined || !Number.isFinite(n) ? '—' : Math.round(n).toLocaleString();

const seconds = (s: number): string =>
  s < 60 ? `${s.toFixed(1)} s` : `${Math.floor(s / 60)} min ${Math.round(s % 60)} s`;

const roughSeconds = (s: number): string => {
...
function renderCorpora(): void {
  ...
  for (const key of Object.keys(CORPORA)) {
    ...
    button.addEventListener('click', () => {
      state.corpusKey = key;
      $<HTMLTextAreaElement>('corpus').value = corpus.text;
      renderCorpora();
      refreshCorpusStats();
      track('corpus_selected', { corpus: key });
    });
    if (key === state.corpusKey) button.classList.add('is-on');
  ...
  own.addEventListener('click', () => {
    state.corpusKey = 'own';
    $<HTMLTextAreaElement>('corpus').focus();
    renderCorpora();
    refreshCorpusStats();
...
function drawLoss(): void {
  const canvas = $<HTMLCanvasElement>('lossChart');
  ...
  const y = (v: number): number => height - 4 - ((v - min) / span) * (height - 12);
...
function startTraining(): void {
  ...
  $<HTMLButtonElement>('trainBtn').disabled = true;
  $<HTMLButtonElement>('stopBtn').disabled = false;
  $<HTMLButtonElement>('generateBtn').disabled = true;
  $<HTMLButtonElement>('downloadWeights').disabled = true;
  $<HTMLButtonElement>('downloadReport').disabled = true;
  $('samples').textContent = '';
...
function onStarted(message: Extract<HostMessage, { type: 'started' }>): void {
  ...
  setStatus(
    message.truncated
      ...
      : 'Training…',
    message.truncated ? 'warn' : ''
  );
}

function onProgress(message: Extract<HostMessage, { type: 'progress' }>): void {
...
function onDone(
  message: Extract<HostMessage, { type: 'done' | 'stopped' }>,
  stopped: boolean
): void {
  state.training = false;
  $<HTMLButtonElement>('trainBtn').disabled = false;
  ...
  setStatus(
    stopped
      ...
      : `Trained ${message.step.toLocaleString()} steps in ${seconds(elapsed)}. Test it below.`,
    'ok'
  );
  if (!stopped) {
    track('model_trained', {
...
function generateSamples(): void {
  ...
  transport?.send({
    type: 'generate',
    prompt: $<HTMLInputElement>('prompt').value,
    length: Number($<HTMLInputElement>('length').value),
    temperature: Number($<HTMLInputElement>('temperature').value),
    topK: Number($<HTMLInputElement>('topK').value),
    count: 3,
  });
}
...
function onWeights(payload: Extract<HostMessage, { type: 'weights' }>['payload']): void {
...
function onReport(payload: Extract<HostMessage, { type: 'report' }>['payload']): void {
...
async function boot(): Promise<void> {
  renderCorpora();
  renderPresets();
  $<HTMLTextAreaElement>('corpus').value = CORPORA[DEFAULT_CORPUS].text;
  refreshCorpusStats();
  drawLoss();
  ...
  $<HTMLTextAreaElement>('corpus').addEventListener('input', () => {
